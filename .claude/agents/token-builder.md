...
9. **Update flow** (`intent: "update"`) — patch in place when name matches. Include every change in the report (added/modified/removed). Rename requires both old + new; one side only → flag and skip.
