...
- **Skills install/strip** — the wizard resolves the active skill set from your stack choices and **deletes every other skill directory** under `.claude/skills/` and `.agents/skills/`. The audit lives in `config.json` under `skillsInstall`. See `.figma-pipeline/protocols/skills.md`.
